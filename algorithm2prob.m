function [result_population, data] = algorithm2(parameters, population)
    ...
    for generation = 1:generations
        ...
        for i = 1:2:pop_size
            ...
            if d1 <= d2
                thresh1 = new_scores(i) / (new_scores(i) + scores(i));
                thresh2 = new_scores(i+1) / (new_scores(i+1) + scores(i+1));
                if rand(1) < thresh1
                ...
                thresh1 = new_scores(i+1) / (new_scores(i+1) + scores(i));
                thresh2 = new_scores(i) / (new_scores(i) + scores(i+1));
                if rand(1) < thresh1
                   new_pop2 = cat(1, new_pop2, new_pop(i + 1, :)); 
                else
                   new_pop2 = cat(1, new_pop2, pop(i, :)); 
